...
export default function AuctionPanel() {
  ...
  const actionsDisabled =
    !isTimerStarted || timer === 0 || isPaused || !!pendingSwitch;

  const switchWinner = pendingSwitch
    ? participants.find(
        (participant) =>
          String(participant.id) === String(pendingSwitch.winnerId),
      )
    : null;

  const handleAdminSwitch = async (candidateId) => {
    try {
  ...
  const switchCard = pendingSwitch ? (
    <div
      style={{
        ...
        background: "linear-gradient(145deg, #241504, #100902)",
      }}
    >
      <div
        style={{
      ...
      <div style={{ color: COLORS.white, fontWeight: "900" }}>
        {pendingSwitch.player?.nome} aggiudicato a {pendingSwitch.price} FM
      </div>

      <div style={{ color: COLORS.muted, marginTop: "6px", marginBottom: "12px" }}>
        Vincitore: <strong style={{ color: COLORS.gold }}>{pendingSwitch.winnerName}</strong>
        <br />
        Saldo: <strong style={{ color: COLORS.green }}>{switchWinner?.crediti ?? 0} FM</strong>
        <br />
        Scegli il calciatore da svincolare:
      </div>

      <div style={{ display: "grid", gap: "8px" }}>
        {(pendingSwitch.switchCandidates || []).map((candidate) => {
          const budget =
            Number(switchWinner?.crediti || 0) + Number(candidate.prezzo || 0);
          const enabled = budget >= Number(pendingSwitch.price || 0);
          ...
          return (
            <button
              ...
              type="button"
              disabled={!enabled}
              onClick={() => handleAdminSwitch(candidate.id)}
              style={{
                width: "100%",
                padding: "11px 12px",
                borderRadius: "10px",
                ...
                color: enabled ? COLORS.white : "#64748b",
