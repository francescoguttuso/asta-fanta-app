    ...
    <div className="card auction-card">
      ...
      {pendingSwitch && (
        ...
        >
          <h3 style={{ color: "#fbbf24", marginTop: 0 }}>
            🔄 TAGLIO CONTESTUALE NECESSARIO
          </h3>
          <p style={{ marginBottom: "6px" }}>
            <strong>{pendingSwitch.winnerName}</strong> ha acquistato
            <strong> {pendingSwitch.player?.nome}</strong> a
            <strong style={{ color: "#10b981" }}> {pendingSwitch.price} FM</strong>.
          </p>
          <p style={{ color: "#94a3b8" }}>
            Saldo: <strong>{pendingSwitch.saldoCrediti} FM</strong> — scegli il giocatore da svincolare.
          </p>

          <div style={{ display: "flex", flexDirection: "column", gap: "7px" }}>
            {(pendingSwitch.switchCandidates || []).map((candidate) => {
              const enabled =
                Number(pendingSwitch.saldoCrediti || 0) +
                  Number(candidate.prezzo || 0) >=
                Number(pendingSwitch.price || 0);
              ...
              return (
                ...
                >
                  ...
                  <span style={{ float: "right" }}>{enabled ? "SWITCH" : "🔒"}</span>
                </button>
              );
            })}
          </div>
      ...
      {player ? (
        <div>
          ...
          <button
            onClick={onAssign}
            disabled={!lastBidderId || !!pendingSwitch}
            className="btn btn-green"
            style={{
              width: "100%",
              padding: "12px",
